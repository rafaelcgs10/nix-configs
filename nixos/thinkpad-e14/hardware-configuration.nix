...
in
{
  ...
  systemd.services.autorandr = {
    ...
    serviceConfig = {
      ExecStart = lib.mkForce "${nixpkgs.autorandr}/bin/autorandr -c --batch";
  ...
  hardware.opengl.extraPackages = with nixpkgs; [
    amdvlk
    # rocm-opencl-icd
    # rocm-opencl-runtime
  ];

  hardware.opengl.extraPackages32 = with nixpkgs; [
  ...
  systemd.user.services.xmodmap = {
    ...
    script = ''
      ${nixpkgs.xorg.xmodmap}/bin/xmodmap -e "keycode 107 = Menu"
  '';
    wantedBy = [ "graphical-session.target" ];
    partOf = [ "graphical-session.target" ];
  };
  # For mount.cifs, required unless domain name resolution is not needed.
  environment.systemPackages = [ nixpkgs.cifs-utils ];
  ...
  # boot.kernelPackages = nixpkgs.linuxPackages_zen;
  ...
  # services.printing.drivers = [ nixpkgs.hplipWithPlugin nixpkgs.gutenprint nixpkgs.gutenprintBin nixpkgs.foomatic-db-ppds-withNonfreeDb nixpkgs.cups-drv-rastertosag-gdi nixpkgs.canon-cups-ufr2 ];
  ...
  #   ""
  #   "${nixpkgs.bluez}/libexec/bluetooth/bluetoothd --noplugin=sap,avrcp"
  # ];
  ...
  systemd.services.hibernate-on-low-battery =
    let
      battery-level-sufficient = nixpkgs.writeShellScriptBin
  ...
  # services.xserver.desktopManager.kodi.package = nixpkgs.kodi.withPackages (p: with p; [ future osmc-skin jellyfin inputstream-rtmp inputstreamhelper inputstream-adaptive inputstream-ffmpegdirect requests myconnpy dateutil invidious joystick ]);
